...
def iniciarEstandar(celula):
    ...
    - Borra completamente las filas de la célula indicada del dataset.
    ...
    try:
	    ...
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] INICIO - celula={}, referencia={}, celulaLinea={}".format(
	        celula, referencia, celulaLinea))
	    ...
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] datasetNuevo generado: {} filas".format(
	        datasetNuevo.getRowCount() if datasetNuevo else 0))
	    ...
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] dataset existente: {} filas, columnas: {}".format(
	        existingDataset.getRowCount(), columns))
	
	    #---Filtrar las filas que NO sean de esta célula (usando strip para evitar problemas de espacios)
	    filasMantener = []
	    filasEliminadas = 0
	    for row in range(existingDataset.getRowCount()):
	        celulaExisting = existingDataset.getValueAt(row, "celula")
	        # Normalizar para comparación: strip espacios y comparar
	        if celulaExisting is not None:
	            celulaExisting = str(celulaExisting).strip()
	        if celulaExisting != celula.strip():
	            filasMantener.append([existingDataset.getValueAt(row, col) for col in columns])
	        else:
	            filasEliminadas += 1
	
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] Filas eliminadas ({}): {}, Filas mantenidas: {}".format(
	        celula, filasEliminadas, len(filasMantener)))
	    ...
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] Total filas final: {}".format(len(filasMantener)))
	    ...
	    system.util.getLogger("G_PILOT_DEBUG").info("[iniciarEstandar] FIN - Tag escrito: {}".format(path))

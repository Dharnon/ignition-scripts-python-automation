...
def completarTareaBD(celula, referencia, maquina, tarea, elemento, manual):
	...
	# Primero obtenemos todas las filas candidatas por celula/referencia/maquina
	# y luego buscamos en Python la fila cuya columna 'tarea' empiece por el valor solicitado
	query = """
		SELECT id, tarea
		FROM {0}
		WHERE celula = ?
	...
	# Filtrar en Python por coincidencia de prefijo en la columna 'tarea'
	if not data or len(data) == 0:
		logger = system.util.getLogger('Tareas.Data.General.completarTareaBD')
		logger.warn("No resumen rows for celula='{}', referencia='{}', maquina='{}'".format(celula, referencia, maquina))
		return False

	idTareasResumen = None
	for row in data:
		# row puede venir como tuple (id, tarea) o como dict
		try:
			id_candidate = row[0]
			tarea_bd = row[1]
		except Exception:
		...
		if tarea_bd and str(tarea_bd).startswith(str(tarea)):
			idTareasResumen = id_candidate
			break

	if idTareasResumen is None:
		logger = system.util.getLogger('Tareas.Data.General.completarTareaBD')
		logger.warn("No matching resumen row (by tarea prefix) for tarea='{}', celula='{}', referencia='{}', maquina='{}'".format(tarea, celula, referencia, maquina))
		return False
...
def iniciarTareaBD(celula, referencia, maquina, tarea, elemento):
	...
	data = system.db.runPrepQuery(query, params, database)
	# Comprobación: si no hay filas, loguear y salir sin insertar
	if not data or len(data) == 0:
		logger = system.util.getLogger('Tareas.Data.General.iniciarTareaBD')
		logger.warn("No matching resumen row for tarea='{}', celula='{}', referencia='{}', maquina='{}'".format(tarea, celula, referencia, maquina))
		return False
